  ...
  def find_user(auth_type)
    ...
    if @user.persisted?
      ...
      if auth_type == 'google_oauth2'
        ...
        if (request.env['omniauth.auth']['info']['email'].end_with? 'gmail.com')
          user_session[:virtual_groups] << 'member@gmail.com'
        end
      end
    ...
    if params['login_popup'].present? || (request.env["omniauth.params"].present? && request.env["omniauth.params"]["login_popup"].present?)
      ...
      redirect_to root_url(:port => nil, :only_path => false, :protocol => 'http')
    end
  end
